package com.andrei1058.amongusmc.arena;

import ch.jalu.configme.SettingsManager;
import ch.jalu.configme.SettingsManagerBuilder;
import com.andrei1058.amongusmc.AmongUsMc;
import com.andrei1058.amongusmc.api.arena.AmongUsArenaManager;
import com.andrei1058.amongusmc.api.arena.Arena;
import com.andrei1058.amongusmc.api.event.GameInitializedEvent;
import com.andrei1058.amongusmc.api.world.WorldAdapter;
import com.andrei1058.amongusmc.arena.command.GameCmd;
import com.andrei1058.amongusmc.arena.command.ForceStartCmd;
...
public class ArenaManager implements AmongUsArenaManager {
    ...
    private static File arenaDirectory = new File(AmongUsMc.getInstance().getDataFolder(), "Templates");
    ...
    private ArenaManager() {
        ...
        if (!newPath.isEmpty()) {
            ...
            if (newDirectory.isDirectory()) {
                ...
                AmongUsMc.getInstance().getLogger().info("Set templates path to: " + newPath);
            } else {
                AmongUsMc.getInstance().getLogger().warning("Tried to set templates path to: " + newPath + " but it does not seem like a directory.");
            }
        }
                    ...
                    if (file.getName().endsWith(".yml")) {
                        ...
                        if (INSTANCE.validateTemplate(templateName)) {
                            ...
                            if (template.getProperty(ArenaConfig.LOAD_AT_START_UP)) {
                                AmongUsArena arena = new AmongUsArena(templateName, INSTANCE.getNextGameId());
                                INSTANCE.addToEnableQueue(arena);
                                if (availableAtOnce > 1) {
                                    for (int i = 1; i < availableAtOnce; i++) {
                                        AmongUsArena clonedArena = new AmongUsArena(templateName, INSTANCE.getNextGameId());
                                        INSTANCE.addToEnableQueue(clonedArena);
                                    }
                                }
                            }
                        } else {
                            AmongUsMc.getInstance().getLogger().warning("Cannot create arena from template: " + templateName);
                        }
                    }
                }
            }
    ...
    public static void onDisable() {
        ...
        AmongUsMc.getInstance().getLogger().info("Disabling arenas..");
        ...
        AmongUsMc.debug("Took " + (System.currentTimeMillis() - startTime) + "ms to disable " + ArenaManager.class.getSimpleName() + ".");
    }
    ...
    public void startArenaFromTemplate(String worldName) {
        AmongUsMc.debug("Starting arena from template: " + worldName);

        // check configuration
        File template = getTemplateFile(worldName);
        if (!template.exists()) {
            AmongUsMc.getInstance().getLogger().warning("Could not load arena from template: " + worldName + ". File does not exist: " + template.getPath());
            return;
        }
        ...
        if (!worldAdapter.hasWorld(worldName)) {
            AmongUsMc.getInstance().getLogger().warning("Could not load arena from template: " + worldName + ". World does not exists in " + worldAdapter.getAdapterName() + "'s container.");
            return;
        }

        // add to enable queue
        Arena arena = new AmongUsArena(worldName, getNextGameId());
    ...
    public boolean addArena(Arena arena) {
        ...
        AmongUsMc.debug("Adding arena with id " + arena.getGameId() + " from template: " + arena.getTemplateWorld() + " to the arenas list.");
    ...
    public boolean addToEnableQueue(Arena arena) {
        ...
        AmongUsMc.debug("Adding to enable queue arena with id " + arena.getGameId() + " from template: " + arena.getTemplateWorld());
        enableQueue.put(gameWorld, arena);
        // a bit of delay to prevent issues when instantiating multiple arenas of the same type
        AmongUsMc.newChain().delay(5 + randomInstance.nextInt(5)).sync(() -> WorldManager.getINSTANCE().getWorldAdapter().onArenaEnableQueue(arena.getTemplateWorld(), arena)).execute();
        return true;
    }
    ...
    public void removeFromEnableQueue(String gameWorld) {
        ...
        AmongUsMc.debug("Removing from enable queue game world: " + gameWorld);
    }
    ...
    public void removeArena(Arena arena) {
        ...
        if (arenas.remove(arena)) {
            AmongUsMc.debug("Removed arena with id " + arena.getGameId() + " from template: " + arena.getTemplateWorld() + " to the arenas list.");
        }
    }
