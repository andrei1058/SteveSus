...
public class RemoteLobby implements RawSocket {
    ...
    public RemoteLobby(String hostAndPort) {
        ...
        if (address.length != 2) {
            AmongUsMc.getInstance().getLogger().warning("Bad RemoteLobby address: " + hostAndPort);
            return;
        }
    ...
    public void connect() {
        ...
        connected = true;
        AmongUsMc.debug("Connected new remote lobby: " + hostAndPort);
        ...
        receiveTask = AmongUsMc.newChain().async(() -> {
            while (connected) {
                if (in.hasNext()) {
                    ...
                    if (receivedString != null) {
                        if (!receivedString.isEmpty()) {
                            ...
                            try {
                                ...
                                AmongUsMc.getInstance().getLogger().warning("Received bad data from: " + socket.getInetAddress().toString() + ". Closing socket..");
                                close();
                                return;
                            }
                            if (json != null) {
                                if (json.has("channel") && json.has("data")) {
                                    ...
                                    PacketChannel channel = AmongUsMc.getInstance().getCommonProvider().getPacketsHandler().getChannelByName(type);
                                    if (channel != null) {
    ...
    public void close() {
        ...
        if (hostAndPort != null) {
            //sockets.remove(hostAndPort);
            AmongUsMc.debug("Closing socket: " + hostAndPort);
        }
        ...
        AmongUsMc.debug("Took " + closeTime + "ms to close slave socket: " + getName());
    }
