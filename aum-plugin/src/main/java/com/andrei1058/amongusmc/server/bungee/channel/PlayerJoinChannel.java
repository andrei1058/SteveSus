package com.andrei1058.amongusmc.server.bungee.channel;

import com.andrei1058.amongusmc.AmongUsMc;
import com.andrei1058.amongusmc.language.LanguageManager;
import com.andrei1058.amongusmc.server.bungee.ProxyUser;
...
public class PlayerJoinChannel implements PacketChannel {
    ...
    public void read(@Nullable RawSocket receiver, JsonObject json) {
        ...
        try {
            ...
            AmongUsMc.debug("Received bad data on " + getClass().getSimpleName());
            return;
        }

        // cache for about 7 seconds for join delays
        AmongUsMc.newChain().sync(() -> new ProxyUser(player, gameId, langIso, target)).execute();
    }
}
