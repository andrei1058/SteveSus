package com.andrei1058.amongusmc.commanditem;
...
public class JoinItemsManager {
    ...
    private JoinItemsManager(File itemsDir) {
        if (!itemsDir.exists()) {
            if (!itemsDir.mkdir()) {
                AmongUsMc.getInstance().getLogger().log(Level.SEVERE, "Could not create " + SelectorManager.getINSTANCE().getSelectorDirectory().getPath());
                return;
            }
        }
        ...
        if (!config.exists()) {
            firstTime = true;
            AmongUsMc.getInstance().getLogger().log(Level.INFO, "Creating " + config.getPath());
            try {
                if (!config.createNewFile()) {
                    AmongUsMc.getInstance().getLogger().log(Level.SEVERE, "Could not create " + config.getPath());
                    return;
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public static void sendCommandItems(@NotNull Player player, @NotNull String category) {
        ...
        AmongUsMc.debug("Giving command items " + category + " to " + player.getUniqueId() + ".");
        ...
        yml.getConfigurationSection(category).getKeys(false).forEach(item -> {
            ...
            if (yml.get(permPath) == null || yml.getString(permPath).isEmpty() || player.hasPermission(yml.getString(permPath))) {
                ...
                if (yml.get(materialPath) != null) {
                    ...
                    AmongUsMc.getInstance().getLogger().warning("Invalid material in join-items config at: " + materialPath);
                }
            }
        });
    }
    ...
    public static void init() {
        if (INSTANCE == null) {
            File itemsDir = AmongUsMc.getInstance().getDataFolder();
            ...
            if (!(customJoinItemsPath = ServerManager.getINSTANCE().getConfig().getProperty(MainConfig.JOIN_ITEMS_PATH)).isEmpty()) {
                ...
                if (newPath.isDirectory()) {
                    itemsDir = newPath;
                    AmongUsMc.getInstance().getLogger().info("Set join-items configuration path to: " + itemsDir);
                } else {
                    AmongUsMc.getInstance().getLogger().warning("Tried to set join-items configuration path to: " + itemsDir + " but it does not seem like a directory.");
                }
            }
