package com.andrei1058.amongusmc.sidebar;
...
public class GameSidebarManager {
    ...
    private GameSidebarManager() throws InstantiationException {
        ...
        if (placeholdersRefreshRate <= 0) {
            AmongUsMc.getInstance().getLogger().warning("Sidebar PLACEHOLDERS refresh is disabled. Refresh interval is set to: " + placeholdersRefreshRate);
        } else {
            if (placeholdersRefreshRate < 20) {
                AmongUsMc.getInstance().getLogger().warning("Sidebar PLACEHOLDERS refresh interval is lower than 20 ticks (1 second).");
                AmongUsMc.getInstance().getLogger().warning("This is not an error but keep in mind it could affect server performance.");
                AmongUsMc.getInstance().getLogger().warning("Current refresh interval: every " + placeholdersRefreshRate + " ticks.");
            }
            Bukkit.getScheduler().runTaskTimer(AmongUsMc.getInstance(), () -> sidebarByPlayer.values().forEach(sidebar -> sidebar.getHandle().refreshPlaceholders()), 20L, placeholdersRefreshRate);
        }
        ...
        if (titleRefreshRate <= 0) {
            AmongUsMc.getInstance().getLogger().warning("Sidebar TITLE refresh is disabled. Refresh interval is set to: " + placeholdersRefreshRate);
        } else {
            if (titleRefreshRate < 20) {
                AmongUsMc.getInstance().getLogger().warning("Sidebar TITLE refresh interval is lower than 20 ticks (1 second).");
                AmongUsMc.getInstance().getLogger().warning("This is not an error but keep in mind it could affect server performance.");
                AmongUsMc.getInstance().getLogger().warning("Current refresh interval: every " + placeholdersRefreshRate + " ticks.");
            }
            Bukkit.getScheduler().runTaskTimer(AmongUsMc.getInstance(), () -> sidebarByPlayer.values().forEach(sidebar -> sidebar.getHandle().refreshTitle()), 20L, titleRefreshRate);
        }
    }
