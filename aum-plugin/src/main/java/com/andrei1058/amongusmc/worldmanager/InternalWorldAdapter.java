...
import com.andrei1058.amongusmc.worldmanager.generator.VoidChunkGenerator;
...
public class InternalWorldAdapter implements WorldAdapter {
    ...
    */
    ...
    private final File backupFolder = new File(AmongUsMc.getInstance().getDataFolder(), "Worlds");
    ...
    public InternalWorldAdapter() {
        ...
        if (!backupFolder.exists()) {
            if (!backupFolder.mkdir()) {
                AmongUsMc.getInstance().getLogger().severe("Could not create directory: " + backupFolder.getPath());
            }
        }
    }
    ...
    public void onAdapterInitialize() {
        ...
        if (dir.exists()) {
            AmongUsMc.getInstance().getLogger().warning("[InternalWorldAdapter] Cleaning obsolete temp worlds...");
    ...
    public void onArenaEnableQueue(String worldToClone, Arena arena) {
        ...
        if (!zipTemplate.exists()) {
            if (worldTemplateFolder.exists()) {
                chain.async(() -> {
                    try {
                        AmongUsMc.getInstance().getLogger().info("Zipping world (backup): " + worldToClone);
                        createZip(zipTemplate, worldTemplateFolder);
                    } catch (ZipException e) {
                        e.printStackTrace();
                        AmongUsMc.getInstance().getLogger().severe("Could not create zip cache for " + worldToClone + "!");
                        AmongUsMc.newChain().sync(() -> ArenaManager.getINSTANCE().removeFromEnableQueue(gameSessionWorld)).execute();
                        chain.abortChain();
                    }
                });
            } else {
                AmongUsMc.getInstance().getLogger().severe(worldToClone + " world was requested by an arena but it was not found!");
        ...
        chain.delay(20).async(() -> {
            try {
                AmongUsMc.getInstance().getLogger().info("Unzipping world backup(" + worldToClone + ") into " + gameSessionWorld + ".");
                if (!unZip(worldToClone, gameSessionWorld)) {
                    AmongUsMc.getInstance().getLogger().severe("Could not unzip cache of  " + worldToClone + "!");
                    AmongUsMc.newChain().sync(() -> ArenaManager.getINSTANCE().removeFromEnableQueue(gameSessionWorld)).execute();
                    chain.abortChain();
                }
            } catch (Exception e) {
        ...
        if (Bukkit.getWorld(gameSessionWorld) == null) {
            ...
            chain.delay(10).sync(() -> {
                try {
                    ...
                    AmongUsMc.getInstance().getLogger().severe("Could not load world: " + gameSessionWorld + " (" + worldToClone + ").");
                    AmongUsMc.newChain().sync(() -> ArenaManager.getINSTANCE().removeFromEnableQueue(gameSessionWorld)).execute();
                    ArenaManager.getINSTANCE().removeFromEnableQueue(gameSessionWorld);
                    chain.abortChain();
                }
    ...
    public void onArenaRestart(Arena arena) {
        ...
        AmongUsMc.newChain().sync(() -> Bukkit.unloadWorld(arena.getWorld(), false)).execute();
    }
    ...
    public void onSetupSessionStart(String worldName, SetupSession setupSession) {
        ...
        if (backupFile.exists()) {
            chain.async(() -> {
                ...
                try {
                    ...
                    templateWorld.mkdir();
                    AmongUsMc.getInstance().getLogger().info("Unzipping world backup: " + worldName);
                    if (!unZip(worldName, setupSession.getWorldName())) {
                        AmongUsMc.getInstance().getLogger().severe("Could not unzip cache for " + worldName + "!");
        ...
        chain.sync(() -> {
            try {
                ...
                AmongUsMc.getInstance().getLogger().severe("Could not load world: " + worldName);
                SetupManager.getINSTANCE().removeSession(setupSession);
                chain.abortChain();
            }
        });
    ...
    public void onSetupSessionClose(SetupSession setupSession) {
        ...
        chain.sync(() -> {
            ...
            if (worldFolder.exists()) {
                try {
                    AmongUsMc.getInstance().getLogger().info("Zipping world (backup): " + setupSession.getWorldName());
                    createZip(zipTemplateFile, worldFolder);
                } catch (ZipException e) {
                    e.printStackTrace();
                    AmongUsMc.getInstance().getLogger().severe("Could not create zip cache for " + setupSession.getWorldName() + "!");
                    chain.abortChain();
                }
            }
        }).async(() -> {
    ...
    private boolean unZip(String worldTemplate, String worldCloneFolder) throws IOException {
        ...
        if (!zipBackup.exists()) {
            AmongUsMc.getInstance().getLogger().warning("Cannot find " + worldTemplate + " folder in " + zipBackup.getAbsolutePath());
            return false;
        }
        ...
        if (worldNameInZip == null && levelFile == null) {
            AmongUsMc.getInstance().getLogger().warning("This doesn't look like a map" + zipBackup.getAbsolutePath());
            return false;
        }
