...
import com.andrei1058.amongusmc.connector.socket.packet.PlayerJoinPacket;
...
import com.andrei1058.amoungusmc.connector.api.event.PlayerGameJoinEvent;
...
public class CachedArena implements RemoteArena {
    ...
    public boolean joinPlayer(Player player, boolean ignoreParty) {
        ...
        if (actualFreeSlots >= requiredSlots) {
            ...
            AmongUsConnector.debug("Sending " + player.getName() + " to game: " + getTag());

            //noinspection UnstableApiUsage
            ByteArrayDataOutput out = ByteStreams.newDataOutput();
            out.writeUTF("Connect");
            out.writeUTF(getServer().getName());
            player.sendPluginMessage(AmongUsConnector.getInstance(), "BungeeCord", out.toByteArray());
    ...
    public boolean joinSpectator(Player player, @Nullable String target) {
        ...
        AmongUsConnector.debug("Sending " + player.getName() + " (as spectator) to game: " + getTag());
