package com.andrei1058.amongusmc.connector.language;
...
import com.andrei1058.amoungusmc.connector.api.event.PlayerLanguageChangeEvent;
...
public class LanguageManager implements CommonLocaleManager {
    ...
    private File languagesFolder = new File(AmongUsConnector.getInstance().getDataFolder(), "Locales");
    ...
    public static void onLoad() {
        ...
        String languagesPath = AmongUsConnector.getConnectorConfig().getProperty(ConnectorConfig.LANGUAGES_FOLDER);
        if (!languagesPath.isEmpty()) {
            File newLanguagesFolder = new File(languagesPath);
            if (!newLanguagesFolder.isDirectory()) {
                AmongUsConnector.getInstance().getLogger().severe("Tried to set languages path to: " + newLanguagesFolder + " but it does not seem to be a directory.");
            } else {
                getINSTANCE().languagesFolder = newLanguagesFolder;
                AmongUsConnector.getInstance().getLogger().info("Set languages path to: " + newLanguagesFolder + ".");
            }
        }
        ...
        if (!INSTANCE.getLocalesFolder().exists()) {
            if (!INSTANCE.getLocalesFolder().mkdir()) {
                AmongUsConnector.getInstance().getLogger().severe("Could not create directory: " + INSTANCE.getLocalesFolder().getPath());
            }
        }
        ...
        String defaultIso = AmongUsConnector.getConnectorConfig().getProperty(ConnectorConfig.FALLBACK_LANGUAGE);
        ...
        if (defaultLanguage != null && getINSTANCE().setDefaultLocale(defaultLanguage)) {
            AmongUsConnector.getInstance().getLogger().info("Set " + defaultIso + " as server's default language!");
            ...
            AmongUsConnector.getInstance().getLogger().severe("Tried to set language " + defaultIso + " as server's default but it seems invalid!");
        }
    }
    ...
    public boolean addLocale(CommonLocale translation) {
        ...
        AmongUsConnector.debug("Adding language: " + translation.getMsg(null, CommonMessage.NAME));
    ...
    public boolean removeLocale(CommonLocale translation) {
        ...
        if (result) {
            AmongUsConnector.debug("Disabled language: " + translation.getMsg(null, CommonMessage.NAME));
        }
        return result;
    }
