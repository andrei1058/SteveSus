...
import com.andrei1058.amongusmc.connector.socket.SocketManager;
...
import com.andrei1058.amoungusmc.connector.api.event.SlaveConnectedEvent;
import com.andrei1058.amoungusmc.connector.api.event.SlaveDisconnectedEvent;
...
public class SlaveServerSocket implements RawSocket {
    ...
    public SlaveServerSocket(@NotNull Socket socket) {
        ...
        AmongUsConnector.debug("Slave server connected: " + socket.toString());

        lastPacket = System.currentTimeMillis();
        connected = true;
        sockets.put(hostAndPort, this);
        AmongUsConnector.newChain().sync(() -> Bukkit.getPluginManager().callEvent(new SlaveConnectedEvent(this))).execute();


        // listen for incoming data
        receiveTask = AmongUsConnector.newChain().async(() -> {
            while (connected) {
                if (in.hasNext()) {
                    ...
                    if (receivedString != null) {
                        if (!receivedString.isEmpty()) {
                            if (receivedString.equalsIgnoreCase("disconnect")) {
                                AmongUsConnector.getInstance().getLogger().warning("Received disconnect packet. Closing socket..");
                                close();
                                return;
                            }
                            ...
                            try {
                                ...
                                AmongUsConnector.getInstance().getLogger().warning("Received bad data from: " + hostAndPort + ". Closing socket..");
                                close();
                                return;
                            }
                            if (json != null) {
                                if (json.has("channel") && json.has("data") && json.has("server")) {
                                    ...
                                    PacketChannel channel = AmongUsConnector.getInstance().getCommonProvider().getPacketsHandler().getChannelByName(type);
                                    if (channel != null) {
                                        if (slaveBungeeName == null) {
                                            if (json.has("server")) {
                                                ...
                                                if (SocketManager.getSlaveByBungeeName(slaveBungeeName) != null) {
                                                    AmongUsConnector.getInstance().getLogger().severe("Denied new slave: " + getHostAndPort() + ". Reason: Bungee name already registered.");
                                                    close();
                                                    return;
                                                }
                                    ...
                                    AmongUsConnector.debug("Received packet (insufficient data) RS: " + receivedString);
                                    close();
                                    return;
                                }
                            } else {
                                AmongUsConnector.debug("Received packet (invalid json) RS: " + receivedString);
                                close();
                                return;
                            }
                        }
    ...
    public void close() {
        if (hostAndPort != null) {
            sockets.remove(hostAndPort);
            AmongUsConnector.debug("Closing socket: " + hostAndPort);
        ...
        AmongUsConnector.newChain().sync(() -> Bukkit.getPluginManager().callEvent(new SlaveDisconnectedEvent(this))).execute();
        ...
        AmongUsConnector.newChain().sync(() -> toRemoveList.forEach(toRemove -> ArenaManager.getInstance().remove(toRemove))).execute();
