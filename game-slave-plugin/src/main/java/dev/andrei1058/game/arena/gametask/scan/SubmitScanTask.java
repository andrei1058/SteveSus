...
public class SubmitScanTask extends GameTask {
    ...
    public SubmitScanTask(double radius, int scanDuration, Location capsuleLocation, GameArena gameArena, String localName) {
        super(localName);
        ...
        gameArena.registerGameListener(new ScanListener());
    }
    ...
    public void onInterrupt(Player player, GameArena gameArena) {
        if (isDoingTask(player)) {
            cancelScan(false, player, gameArena);
        }
    }
     ...
     */
    private boolean startScan(Player player, GameArena gameArena) {
        if (currentScan != null) return false;
        if (!gameArena.isTasksAllowedATM()) return false;
        if (gameArena.getCamHandler() != null && gameArena.getCamHandler().isOnCam(player, gameArena)) return false;
        ...
        currentScanTask = Bukkit.getScheduler().runTaskTimer(SteveSus.getInstance(), () -> {
            if (currentSecond[0] <= 0) {
                cancelScan(true, player, gameArena);
            } else {
                currentSecond[0]--;
                ...
                if (gameArena.getLiveSettings().isVisualTasksEnabled() && getHandler().isVisual() && (playerTeam == null || !playerTeam.getIdentifier().endsWith("-ghost"))) {
    ...
    private void cancelScan(boolean done, Player player, GameArena gameArena) {
