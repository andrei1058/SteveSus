...
import dev.andrei1058.game.common.database.table.LanguageTable;
...
public class LanguageManager implements LocaleManager, CommonLocaleManager {
    ...
    public static void onLoad() {
        ...
        if (!languagesPath.isEmpty()) {
            ...
            if (!newLanguagesFolder.isDirectory()) {
                SteveSus.getInstance().getLogger().severe("Tried to set languages path to: " + newLanguagesFolder + " but it does not seem to be a directory.");
            } else {
                getINSTANCE().languagesFolder = newLanguagesFolder;
                SteveSus.getInstance().getLogger().info("Set languages path to: " + newLanguagesFolder + ".");
            }
        }
        ...
        if (defaultLanguage != null && getINSTANCE().setDefaultLocale(defaultLanguage)) {
            SteveSus.getInstance().getLogger().info("Set " + defaultIso + " as server's default language!");
            ...
            SteveSus.getInstance().getLogger().severe("Tried to set language " + defaultIso + " as server's default but it seems invalid!");
        }
    }
    ...
    public boolean addLocale(Locale translation) {
        ...
        SteveSus.debug("Adding language: " + translation.getRawMsg(CommonMessage.NAME.toString()));
    ...
    public boolean removeLocale(Locale translation) {
        ...
        if (result) {
            SteveSus.debug("Disabled language: " + translation.getMsg(null, CommonMessage.NAME));
        }
        return result;
    }
